...
<ul>
  ...
  <li>Controleer het <a href=\"../../downloads/RenEreglement _april2009.doc\" target=\"_blank\">Roei- en Examenreglement</a> op theorie- en exameneisen.</li>
  <li>Op het <a href=\"../../downloads/beoordelingalleexamens13.pdf\" target=\"_blank\">beoordelingsformulier</a> kunt u zien welke criteria de examinator hanteert bij het afnemen van het examen.</li>
  <li>Je kunt hier de <a href=\"http://hunze.nl/downloads/Examenvragen.doc\" target=\"_blank\">examenvragen</a> T1 en T2 bekijken.</li>
...
De examendata voor de rest van dit jaar zijn:
<ul>
<?php
$thisyear = date('Y');
$query = "SELECT Datum, ToonOpSite FROM examens WHERE YEAR(Datum)=$thisyear AND Datum>'$today_db' ORDER BY Datum;";
...
if (!$result) {
	...
	if ($rows_aff > 0) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			...
			if ($show) echo " (open voor inschrijving)";
			echo "</li>";
		}
	}
}
...
<?php
$query = "SELECT ID, Datum, Quotum, Omschrijving FROM examens WHERE YEAR(Datum)=$thisyear AND Datum>'$today_db' AND ToonOpSite=1 ORDER BY Datum;";
$result = mysql_query($query);
if (!$result) {
	echo("Ophalen van examendata mislukt.".mysql_error());
} else {
